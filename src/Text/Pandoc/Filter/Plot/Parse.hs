...
parseFigureSpec :: (PlotConfig c, RendererM c m) => Block -> m (Maybe FigureSpec)
parseFigureSpec (CodeBlock (id', cls, attrs) content) = do
    ...
    where
        ...
        figureSpec :: (PlotConfig c, RendererM c m) => m FigureSpec
        figureSpec = do
            config <- ask
            extraAttrs' <- parseExtraAttrs attrs'
            let includeScript = mempty -- TODO: use preamble from PlotConfig type as default
