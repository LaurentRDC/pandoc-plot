...
parseFigureSpec :: (RendererConfig c, RendererM c m) => Block -> m (Maybe FigureSpec)
parseFigureSpec (CodeBlock (id', cls, attrs) content) = do
    ...
    where
        ...
        preamblePath  = unpack <$> Map.lookup preambleKey attrs' -- TODO: this
        header        = "# Generated by pandoc-plot " <> ((pack . showVersion) version)

        figureSpec :: (RendererConfig c, RendererM c m) => m FigureSpec
        figureSpec = do
            config <- ask
            extraAttrs' <- parseExtraAttrs attrs'
            let includeScript = mempty -- TODO: use preamble from RendererConfig type as default
